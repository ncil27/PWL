...
-- -----------------------------------------------------
-- Table `pengajuan_surat_if`.`periode`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pengajuan_surat_if`.`periode` ;

CREATE TABLE IF NOT EXISTS `pengajuan_surat_if`.`periode` (
  `id_periode` VARCHAR(15) NOT NULL,
  `nama_periode` VARCHAR(25) NOT NULL,
  PRIMARY KEY (`id_periode`),
  UNIQUE INDEX `id_periode_UNIQUE` (`id_periode` ASC) ,
  UNIQUE INDEX `periode_UNIQUE` (`nama_periode` ASC) )
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `pengajuan_surat_if`.`surat_mhs_aktif` (
  ...
  INDEX `surat_id_idx` (`id_pengajuan` ASC) ,
  INDEX `periode_skma_periode_idx` (`id_periode` ASC) ,
  CONSTRAINT `mhs_surat_id`
    FOREIGN KEY (`id_pengajuan`)
    REFERENCES `pengajuan_surat_if`.`pengajuan` (`id_pengajuan`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `periode_skma_periode`
    FOREIGN KEY (`id_periode`)
    REFERENCES `pengajuan_surat_if`.`periode` (`id_periode`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb3;
...
CREATE TABLE IF NOT EXISTS `pengajuan_surat_if`.`surat_pengantar` (
  ...
  INDEX `surat_id_idx` (`id_pengajuan` ASC) ,
  INDEX `periode_sp_periode_idx` (`id_periode` ASC) ,
  CONSTRAINT `pengantar_surat_id`
    FOREIGN KEY (`id_pengajuan`)
    REFERENCES `pengajuan_surat_if`.`pengajuan` (`id_pengajuan`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `periode_sp_periode`
    FOREIGN KEY (`id_periode`)
    REFERENCES `pengajuan_surat_if`.`periode` (`id_periode`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE)
...
USE `pengajuan_surat_if`$$
DROP TRIGGER IF EXISTS `pengajuan_surat_if`.`generate_id_pengajuan` $$
USE `pengajuan_surat_if`$$
CREATE
DEFINER=`root`@`localhost`
TRIGGER `pengajuan_surat_if`.`generate_id_pengajuan`
BEFORE INSERT ON `pengajuan_surat_if`.`pengajuan`
FOR EACH ROW
BEGIN
    DECLARE NoAkhir INT;
    DECLARE NoBaru INT;
...
USE `pengajuan_surat_if`$$
DROP TRIGGER IF EXISTS `pengajuan_surat_if`.`generate_id_surat_ket_lulus` $$
USE `pengajuan_surat_if`$$
CREATE
DEFINER=`root`@`localhost`
TRIGGER `pengajuan_surat_if`.`generate_id_surat_ket_lulus`
BEFORE INSERT ON `pengajuan_surat_if`.`surat_ket_lulus`
FOR EACH ROW
BEGIN
...
USE `pengajuan_surat_if`$$
DROP TRIGGER IF EXISTS `pengajuan_surat_if`.`generate_id_surat_lhs` $$
USE `pengajuan_surat_if`$$
CREATE
DEFINER=`root`@`localhost`
TRIGGER `pengajuan_surat_if`.`generate_id_surat_lhs`
BEFORE INSERT ON `pengajuan_surat_if`.`surat_lhs`
FOR EACH ROW
BEGIN
    DECLARE NoAkhir INT DEFAULT 0;
    DECLARE NoBaru INT;
...
USE `pengajuan_surat_if`$$
DROP TRIGGER IF EXISTS `pengajuan_surat_if`.`periode_BEFORE_INSERT` $$
USE `pengajuan_surat_if`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pengajuan_surat_if`.`periode_BEFORE_INSERT` BEFORE INSERT ON `periode` FOR EACH ROW
BEGIN
    DECLARE jenis varchar(7);
  DECLARE jns varchar(2);
  DECLARE nama varchar(15);
  DECLARE nm varchar(3);
  DECLARE id VARCHAR(15);
  DECLARE tahun varchar(9);
  DECLARE thn1 varchar(2);
  DECLARE thn2 varchar(2);

  set jenis = LOWER(SUBSTRING_index(NEW.nama_periode," ",1)); -- dari Reguler Ganjil XXXX/XXXX diambil Reguler
  if (STRCMP(jenis,"reguler")=0)
  then set jns = "R";
  elseif (STRCMP(jenis,"antara")=0)
  then set jns = "SA";
  else set jns="X";
  end if;

  set nama = lower(SUBSTRING_index(NEW.nama_periode," ",2)); -- dari Reguler Ganjil XXXX/XXXX diambil reguler ganjil
  set nama = SUBSTRING_index(nama," ",-1); -- dari reguler ganjil diambil ganjil
  if (STRCMP(nama,"genap")=0)
  then set nm = "GNP";
  elseif (STRCMP(nama,"ganjil")=0)
  then set nm = "GNJ";
  else set nm="XXX";
  end if;

  set tahun = substring_index(NEW.nama_periode," ",-1);
  set thn1 = substring(tahun,3,2);
  set thn2 = substring(tahun,-2,2);

  -- Format ID: PGN-DDMMYYY-0001
  SET id = CONCAT(jns,"-", nm,thn1,"/",thn2);

  -- Set nilai id_pengajuan baru
  SET NEW.id_periode = id;
END$$


USE `pengajuan_surat_if`$$
DROP TRIGGER IF EXISTS `pengajuan_surat_if`.`generate_id_surat_mhs_aktif` $$
USE `pengajuan_surat_if`$$
CREATE
DEFINER=`root`@`localhost`
TRIGGER `pengajuan_surat_if`.`generate_id_surat_mhs_aktif`
BEFORE INSERT ON `pengajuan_surat_if`.`surat_mhs_aktif`
FOR EACH ROW
BEGIN
    DECLARE NoAkhir INT DEFAULT 0;
    DECLARE NoBaru INT;
...
USE `pengajuan_surat_if`$$
DROP TRIGGER IF EXISTS `pengajuan_surat_if`.`generate_id_surat_pengantar` $$
USE `pengajuan_surat_if`$$
CREATE
DEFINER=`root`@`localhost`
TRIGGER `pengajuan_surat_if`.`generate_id_surat_pengantar`
BEFORE INSERT ON `pengajuan_surat_if`.`surat_pengantar`
FOR EACH ROW
BEGIN
    DECLARE NoAkhir INT DEFAULT 0;
    DECLARE NoBaru INT;
...
-- -----------------------------------------------------
-- Data for table `pengajuan_surat_if`.`periode`
-- -----------------------------------------------------
START TRANSACTION;
USE `pengajuan_surat_if`;
INSERT INTO `pengajuan_surat_if`.`periode` (`id_periode`, `nama_periode`) VALUES ('R-GNJ24/25', 'Reguler Ganjil 2024/2025');
INSERT INTO `pengajuan_surat_if`.`periode` (`id_periode`, `nama_periode`) VALUES ('R-GNP24/25', 'Reguler Genap 2024/2025');
INSERT INTO `pengajuan_surat_if`.`periode` (`id_periode`, `nama_periode`) VALUES ('SA-GNP24/25', 'Antara Genap 2024/2025');
